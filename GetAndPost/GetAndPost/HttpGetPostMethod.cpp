...
//
//  select函数用法：
//  http://www.cnblogs.com/ggzss/archive/2011/08/25/2153746.html
...
std::string HttpGetPostMethod::HttpSocket(std::string host, std::string request_str)
{
    ...
    switch(server_hostent->h_addrtype) {
        case AF_INET:
        case AF_INET6:
            pptr = server_hostent->h_addr_list;
            for(; *pptr != nullptr; pptr++) {
                cout << "IP Addr: " << inet_ntop(server_hostent->h_addrtype, *pptr, str, sizeof(str)) << endl;
            }
    }
    ...
    // 将POST或者GET请求发送出去
    ret = (int)send(client_fd, request_str.c_str(), request_str.length(), 0);
    ...
    while(1) {
        ...
        if(FD_ISSET(client_fd, &client_fd_set)) {    // FD_ISSET(s,*set)：若s为集合中一员，非零；否则为零
            bzero(recv_buf, kBufferSize);
            ret = (int)read(client_fd, recv_buf, kBufferSize);
